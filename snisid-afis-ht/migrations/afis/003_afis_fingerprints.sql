...
CREATE TABLE afis_fingerprints (
    ...
    subject_id          UUID NOT NULL REFERENCES afis_subjects(subject_id) ON DELETE CASCADE,
...
CREATE INDEX idx_afis_prints_milvus ON afis_fingerprints(milvus_vector_id) WHERE milvus_vector_id IS NOT NULL;
CREATE INDEX idx_afis_prints_primary ON afis_fingerprints(subject_id) WHERE is_primary = TRUE;
