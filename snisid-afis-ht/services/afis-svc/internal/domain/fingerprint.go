...
var (
	ErrQualityTooLow          = errors.New("qualité empreinte insuffisante: NFIQ2 score < 60")
	ErrMissingRequiredFingers = errors.New("empreintes obligatoires manquantes (pouces + index requis)")
	ErrSubjectNotFound        = errors.New("sujet AFIS non trouvé")
	ErrPrintNotFound          = errors.New("empreinte non trouvée")
)
...
func (f *Fingerprint) IsHighQuality() bool {
	return f.NFIQ2Score >= 80
}
...
func (f *Fingerprint) Validate() error {
	...
	if f.ImageRef == "" {
		return errors.New("référence image manquante")
	}
	return nil
}
...
type Subject struct {
	...
	AliasIDs         []uuid.UUID  `json:"alias_ids" db:"alias_ids"`
...
type EnrollmentRequest struct {
	...
	Fingerprints    []FingerprintCapture `json:"fingerprints" validate:"required,min=2,dive"`
}
...
type SearchRequest struct {
	...
	LatentID        *uuid.UUID      `json:"latent_id,omitempty"`
	Fingerprints    []FingerprintCapture `json:"fingerprints,omitempty"`
}

type LatentSubmitRequest struct {
...
type LatentMatchRequest struct {
	LatentID       uuid.UUID `json:"latent_id" validate:"required"`
	SubjectID      uuid.UUID `json:"subject_id" validate:"required"`
	MatchScore     float64   `json:"match_score" validate:"required,min=0,max=100"`
	ConfirmedBy    uuid.UUID `json:"confirmed_by" validate:"required"`
}