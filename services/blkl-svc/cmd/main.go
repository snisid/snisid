...
func main() {
	...
	dbSSLMode := getEnv("BLKL_DB_SSLMODE", "disable")
	kafkaBrokers := getEnv("BLKL_KAFKA_BROKERS", "localhost:9092")
	kafkaTopic := getEnv("BLKL_KAFKA_TOPIC", "snisid.blkl.events")
	...
	go func() {
		logger.Info("blkl-svc started", zap.String("port", port))
