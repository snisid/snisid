...
func (m *DriftMonitor) DetectCollapseRisk(current float64) bool {
	...
	if drift > 0.25 {
		logger.Warn(context.Background(), "SELF-REGULATION: High system drift detected. Initiating Safety Freeze.")
		return true
	}
	return false
}
...
func SafetyFreeze() {
	logger.Error(context.Background(), "SAFETY: RL-Optimization suspended. Reverting to Baseline Stability Model.", nil)
}
