...
import (
	...
	"github.com/snisid/platform/services/gang-svc/internal/handler"
	"github.com/snisid/platform/services/gang-svc/internal/kafka"
...
func main() {
	port := getEnv("GANG_SERVICE_PORT", "8095")
	dbURL := getEnv("GANG_DB_URL", "postgresql://root@localhost:26257/snisid_gang?sslmode=disable")
	kafkaBrokers := getEnv("GANG_KAFKA_BROKERS", "localhost:9092")
	...
	incRepo := postgres.NewIncidentRepo(pool)
	alliRepo := postgres.NewAllianceRepo(pool)

	eventPub := &kafkaEventPublisher{producer: producer}

	orgSvc := service.NewOrganizationService(orgRepo, incRepo, alliRepo, eventPub)
	incSvc := service.NewIncidentService(incRepo, orgRepo, eventPub)
	alliSvc := service.NewAllianceService(alliRepo, orgRepo, eventPub)

	h := handler.NewHandler(orgSvc, incSvc, alliSvc)
	...
	go func() {
		log.Printf("GANG-HT service starting on port %s", port)
