...
import (
	...
	"github.com/snisid/platform/services/sipep-svc/internal/handler"
	"github.com/snisid/platform/services/sipep-svc/internal/kafka"
...
func main() {
	port := getEnv("SIPEP_SERVICE_PORT", "8092")
	dbURL := getEnv("SIPEP_DB_URL", "postgresql://root@localhost:26257/snisid_sipep?sslmode=disable")
	kafkaBrokers := getEnv("SIPEP_KAFKA_BROKERS", "localhost:9092")
	...
	inmateRepo := postgres.NewInmateRepo(pool)
	detentionRepo := postgres.NewDetentionRepo(pool)
	transferRepo := postgres.NewTransferRepo(pool)

	eventPub := &kafkaEventPublisher{producer: producer}

	intakeSvc := service.NewIntakeService(inmateRepo, detentionRepo, eventPub, nil)
	...
	overcrowdingSvc := service.NewOvercrowdingService(nil)

	h := handler.NewHandler(intakeSvc, releaseSvc, transferSvc, overcrowdingSvc)
	...
	go func() {
		log.Printf("SIPEP-HT service starting on port %s", port)
