...
export function verifyAccessToken(token: string): AuthenticatedPrincipal {
  ...
  if (decoded.sessionId) principal.sessionId = decoded.sessionId;
  return principal;
}
...
export function signServiceToken(principal: AuthenticatedPrincipal): string {
  return jwt.sign(
    ...
    { issuer: env.JWT_ISSUER, audience: env.JWT_AUDIENCE, expiresIn: env.JWT_EXPIRES_IN } as jwt.SignOptions
  );
}
